export function getAppointmentsForDay(state, day) {
  ...
  const result = appointmentsArray.filter((appointment) =>
    filteredDaysAppointmentsId.includes(appointment.id)
  );
  return result;
}
...
export function getInterviewersForDay(state, day) {
  ...
  const result = interviewersArray.filter((interviewer) =>
    filteredDaysInterviewersId.includes(interviewer.id)
  );
  return result;
}
